#include "lilygo_tkey_s3p.h"
#include "esphome/core/log.h"
#include <cstdio>

namespace esphome::lilygo_tkey_s3p {

static const char *const TAG = "lilygo_tkey_s3p";

void LilyGoTKeyboardS3Pro::setup() {
  uint16_t iocon;
  ...
  if (this->interrupt_pin_ != nullptr) {
    ...
    this->interrupt_pin_->attach_interrupt(&LilyGoTKeyboardS3Pro::gpio_intr, this, gpio::INTERRUPT_FALLING_EDGE);
    this->set_invalidate_on_read_(false);
  }
  this->disable_loop();
}

void IRAM_ATTR LilyGoTKeyboardS3Pro::gpio_intr(LilyGoTKeyboardS3Pro *arg) {
  arg->enable_loop_soon_any_context();
}

void LilyGoTKeyboardS3Pro::loop() {
...
bool LilyGoTKeyboardS3Pro::digital_read_hw(uint8_t pin) {
  return this->read_reg_(MCP23016_GP1, &this->input_mask_);
}

bool LilyGoTKeyboardS3Pro::digital_read_cache(uint8_t pin) {
  return this->input_mask_ & (1 << pin);
}

void LilyGoTKeyboardS3Pro::digital_write_hw(uint8_t pin, bool value) {
  this->update_reg_(pin, value, MCP23016_OLAT1);
}

void LilyGoTKeyboardS3Pro::pin_mode(uint8_t pin, gpio::Flags flags) {
...
float LilyGoTKeyboardS3Pro::get_setup_priority() const {
  return setup_priority::IO;
}

bool LilyGoTKeyboardS3Pro::read_reg_(uint8_t reg, uint16_t *value) {
  if (this->is_failed())
    return false;
...
bool LilyGoTKeyboardS3Pro::write_reg_(uint8_t reg, uint16_t value) {
  if (this->is_failed())
    return false;
...
void LilyGoTKeyboardS3Pro::update_reg_(uint8_t pin, bool pin_value, uint8_t reg_addr) {
